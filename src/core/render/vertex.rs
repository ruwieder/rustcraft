    ...
    pub const fn desc() -> wgpu::VertexBufferLayout<'static> {
        wgpu::VertexBufferLayout { 
            ...
            attributes: &[
                ...
                wgpu::VertexAttribute { // texture_coords
...
const CUBE_VERTICES: &[Vertex] = &[ // Note: this tex_coords is not used
...
pub fn generate_voxel_face(pos: Vector3<f32>, color: [f32; 3], normal: Vector3<f32>, texture_id: u16) -> (Vec<Vertex>, Vec<u32>) {
    ...
    const TEX_OFFSET_U: f32 = 0.0;
    const TEX_OFFSET_V: f32 = 6.0 / ATLAS_HEIGHT;
    
    debug_assert!((texture_id as f32) < TEXTURES_PER_COL * TEXTURES_PER_ROW);
    
    const HALF: f32 = 0.5; // NOTE: should be 0.5 for no gaps
    
    let tex_x = (texture_id as f32 % TEXTURES_PER_ROW) * TEX_SIZE_U + TEX_OFFSET_U;
    let tex_y = (texture_id as f32 / TEXTURES_PER_COL) * TEX_SIZE_V + TEX_OFFSET_V;
