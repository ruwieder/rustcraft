...
use cgmath::Vector3;
use crate::core::{render::vertex::generate_voxel_mesh, *};
    ...
    pub fn load_chunks(&mut self, x: i64, y: i64, z: i64) {
        ...
        if !self.chunks.contains_key(&key) {
            self.chunks.insert(key, 
                Chunk::new_fill(Vector3::new(x, y, z), (100, 100, 100))
            );
        };
    }
                ...
                for y in 0..CHUNK_SIZE {
                    for z in 0..CHUNK_SIZE {
                        ...
                        let world_pos = Vector3::new(
                            chunk.pos.x as f32 + x as f32,
                            chunk.pos.y as f32 + x as f32,
                            chunk.pos.z as f32 + x as f32,
                        );
                        if self.is_face_exposed(world_pos, Vector3::new(-1.0, 0.0, 0.0)) {
                            let (mut v, mut i) = generate_voxel_mesh(world_pos, color);
                            for vertex in &mut v {
                                vertex.pos[0] += 0.5;
                                vertex.pos[1] += 0.5;
                                vertex.pos[2] += 0.5;
